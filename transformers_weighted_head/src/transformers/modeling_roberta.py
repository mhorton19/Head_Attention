...
class RobertaSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.weight_heads = config.weight_heads

        if self.weight_heads:
            self.head_query = nn.Linear(config.hidden_size, self.all_head_size)
            self.head_key = nn.Linear(config.hidden_size, self.all_head_size)
    ...
    def forward(
        ...
        if encoder_hidden_states is not None:
            ...
            hidden_input = encoder_hidden_states
        else:
        ...
        if self.weight_heads:
            head_query_layer = self.transpose_for_scores(self.head_query(hidden_input))
            head_key_layer_unsummed = self.transpose_for_scores(self.head_key(hidden_input))
        ...
        if self.weight_heads:
            head_key_layer = torch.matmul(attention_probs, head_key_layer_unsummed)
            head_attention_scores = torch.matmul(head_query_layer.unsqueeze(-2), head_key_layer.unsqueeze(-1)).squeeze(-1)
            head_attention_scores = head_attention_scores / math.sqrt(self.attention_head_size)

            head_attention_probs = nn.Softmax(dim=1)(head_attention_scores)
            context_layer = context_layer * head_attention_probs
